...
export class MetadataClient {
  ...
  public static async retrieveAllMetadata(host: string): Promise<EndpointMetadata> {
    ...
    if (oid4vciMetadata) {
      ...
      if (!token_endpoint && oid4vciMetadata.auth_service) {
        // Crossword uses this to separate the AS metadata
        const asMetadata: OAuth2ASMetadata = await this.retrieveWellknown(oid4vciMetadata.auth_service, WellKnownEndpoints.OAUTH_AS, { errorOnNotFound: true });
      ...
      let asConfig: Oauth2ASWithOID4VCIMetadata = await MetadataClient.retrieveWellknown(host, WellKnownEndpoints.OIDC_CONFIGURATION, { errorOnNotFound: false });
      if (!asConfig) {
        // Now oAuth2
  ...
  public static async retrieveWellknown<T>(host: string, endpointType: WellKnownEndpoints, opts?: { errorOnNotFound?: boolean }): Promise<T | undefined> {
    try {
