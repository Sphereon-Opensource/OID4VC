...
describe('Credential Request Client with Walt.id ', () => {
  it('should have correct metadata endpoints', async function () {
    ...
    const initiation = CredentialOffer.fromURI(WALT_IRR_URI);

    const metadata = await MetadataClient.retrieveAllMetadataFromCredentialOffer(initiation);
