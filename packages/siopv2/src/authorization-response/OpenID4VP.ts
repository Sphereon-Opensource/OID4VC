import { ResponseType } from '@sphereon/oid4vc-common';
...
export const extractPresentationsFromAuthorizationResponse = async (
  ...
  return presentations.map((presentation) => CredentialMapper.toWrappedVerifiablePresentation(presentation, { hasher: opts?.hasher }));
};
...
export const createPresentationSubmission = async (
  ...
  for (const verifiablePresentation of verifiablePresentations) {
    ...
    if (!submission && opts?.presentationDefinitions) {
      console.warn(`No submission_data in VPs and not provided. Will try to deduce, but it is better to create the submission data beforehand`);
