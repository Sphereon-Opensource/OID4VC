...
describe('RP using test vectors', () => {
  ...
  it('should decode id token jwt', async () => {
    ...
    const xx = JSON.stringify({"credentials": [{"id":"VerifiedEmployeeVC","format":"vc+sd-jwt","meta":{"vct_values":["https://high-assurance.com/VerifiedEmployeeVC"]},"claims":[{"path":["license", "number"]},{"path":["user", "name"]}]}]})
    ...
    const jwtPayload = {
      ...
      "_vp_token": { "dcql_query": "{\"credentials\":[{\"id\":\"VerifiedEmployeeVC\",\"format\":\"vc+sd-jwt\",\"meta\":{\"vct_values\":[\"https://high-assurance.com/VerifiedEmployeeVC\"]},\"claims\":[{\"path\":[\"license\",\"number\"]},{\"path\":[\"user\",\"name\"]}]}]}" }
    }

    // const token = jwt.sign(jwtPayload, "a-string-secret-at-least-256-bits-long", {
    //   algorithm: 'ES256',       // optional, defaults to HS256
    //   header: {
    //     typ: 'JWT',             // typically "JWT"
    //     kid: 'my-key-id'        // your custom header parameter
    //   },
