...
// skipping as we now only should support v1 (final)
describe.skip('OID4VCI-Client using Mattr issuer should', () => {
  async function testWithOp(format: string | string[]) {
    ...
    for (const [key, value] of Object.entries(dcqlQueryResult.credential_matches)) {
      if (value.success) {
        presentation[key] = OPENBADGE_JWT_VC.compactJwtVc
      }
    }
...
// skipping as we now only should support v1 (final)
describe.skip('Mattr OID4VP v18 credential offer', () => {
  test('should verify using request directly', async () => {
    ...
    expect(verification.versions).toEqual([SupportedVersion.SIOPv2_OID4VP_D28, SupportedVersion.OID4VP_v1])
