...
export function base64ToHexString(input: string, encoding?: 'base64url' | 'base64'): string {
  return u8a.toString(u8a.fromString(input, encoding ?? 'base64url'), 'base16')
}
...
export function base64urlToString(base64url: string): string {
  const uint8array = u8a.fromString(base64url, 'base64url')
  return u8a.toString(uint8array, 'ascii')
}
