...
export const verifyPresentations = async (
  ...
  const dcqlQuery = DcqlQuery.parse(verifyOpts.dcqlQuery ?? JSON.parse(authorizationResponse?.authorizationRequest.payload.dcql_query)) // TODO dcql_query is now required?
